...
contract JobMarketplace is ReentrancyGuard, Pausable, Ownable {
    ...
    modifier onlyFreelancer(uint256 _jobId) {
        require(
            ...
            "Only assigned freelancer can call"
        );
        _;
    }
    ...
    function addMilestones(
        ...
        require(
            ...
            "Can only add milestones to open jobs"
        );
        require(
        ...
        require(
            totalMilestoneAmount <= job.totalBudget,
            "Milestones exceed budget"
        );
    }
    ...
    function hireFreelancer(
        uint256 _jobId,
        address _freelancer
    ) external jobExists(_jobId) onlyEmployer(_jobId) whenNotPaused {
        Job storage job = jobs[_jobId];
        require(job.status == JobStatus.Open, "Job is not open");
    ...
    ) external jobExists(_jobId) onlyFreelancer(_jobId) whenNotPaused {
        Job storage job = jobs[_jobId];
        require(job.status == JobStatus.InProgress, "Job not in progress");
        require(
            _milestoneIndex < jobMilestones[_jobId].length,
            "Invalid milestone"
        );
        ...
        whenNotPaused
    {
        ...
        (bool successFreelancer, ) = payable(job.freelancer).call{
        ...
        if (_areAllMilestonesApproved(_jobId)) {
            ...
            emit JobCompleted(
                _jobId,
                job.freelancer,
                job.totalBudget - job.remainingBudget
            );
        }
    }
        ...
        whenNotPaused
    {
        ...
        require(job.status == JobStatus.InProgress, "Job not in progress");
        require(
            jobMilestones[_jobId].length == 0,
            "Use milestone approval for jobs with milestones"
        );
    ...
    function resolveDispute(
        ...
        require(job.remainingBudget > 0, "No funds available");
        require(
            _freelancerAmount <= job.remainingBudget,
        ...
        if (_favorFreelancer) {
            uint256 platformFee = (_freelancerAmount * platformFeePercent) /
                100;
            ...
            if (employerRefund > 0) {
                (bool successEmployer, ) = payable(job.employer).call{
            ...
            job.status = JobStatus.Cancelled;
            (bool success, ) = payable(job.employer).call{
    ...
    function withdrawAfterDeadline(
        uint256 _jobId
    )
        external
        jobExists(_jobId)
        ...
        whenNotPaused
    {
        ...
        require(job.status == JobStatus.InProgress, "Job not in progress");
        require(
            block.timestamp > job.deadline + DISPUTE_TIMEOUT,
            "Deadline not passed"
        );
        require(job.remainingBudget > 0, "No funds available");
    ...
    function rateUser(
        ...
        require(
            (msg.sender == job.employer && _ratee == job.freelancer) ||
                (msg.sender == job.freelancer && _ratee == job.employer),
            "Invalid rater/ratee combination"
        );
        require(!hasRated[_jobId][msg.sender], "Already rated");
        ...
        userTotalRating[_ratee] += _score;
        userRatingCount[_ratee]++;
        hasRated[_jobId][msg.sender] = true;

        emit RatingGiven(_jobId, msg.sender, _ratee, _score);
    }
    ...
    function getJobApplications(
        uint256 _jobId
    ) external view jobExists(_jobId) returns (Application[] memory) {
        return jobApplications[_jobId];
    }

    function getJobMilestones(
        uint256 _jobId
    ) external view jobExists(_jobId) returns (Milestone[] memory) {
    ...
    function _areAllMilestonesApproved(
        uint256 _jobId
